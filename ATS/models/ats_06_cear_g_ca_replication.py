...
WEIGHT_DECAY = 1e-4
GRAD_CLIP = 1.0
PATIENCE = 0  # 0 = disabled; replication run completes all requested epochs
MIN_DELTA = 5e-4  # only used when PATIENCE > 0
CHECKPOINT_EVERY = 1  # model weights every epoch
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "ATS 06: CEAR-style recurrent perspective latent g on "
            "Community Alignment."
        ),
    ...
    p.add_argument("--coupler", choices=["metric", "film"], default=COUPLER)
    p.add_argument("--g-dim", type=int, default=G_DIM)
    p.add_argument("--metric-rank", type=int, default=METRIC_RANK)
    ...
    p.add_argument("--min-delta", type=float, default=MIN_DELTA,
                   help="Minimum validation improvement used only for early stopping.")
    p.add_argument("--checkpoint-every", type=int, default=CHECKPOINT_EVERY,
                   help="Save model-only epoch checkpoint every N epochs.")
    ...
    p.add_argument("--embed-batch-size", type=int, default=EMBED_BATCH_SIZE)
    p.add_argument("--val-permutations", type=int, default=VAL_PERMUTATIONS)
    p.add_argument("--test-permutations", type=int, default=TEST_PERMUTATIONS)
    p.add_argument("--bootstrap", type=int, default=N_BOOTSTRAP,
                   help="Number of user-cluster bootstrap resamples.")
    p.add_argument("--text-encoder", default=TEXT_ENCODER)
    p.add_argument("--cache-dir", default=str(CACHE_ROOT),
                   help="Absolute path or path relative to repository root.")
    p.add_argument("--result-dir", default=str(RESULT_ROOT),
                   help="Absolute path or path relative to repository root.")
    p.add_argument("--hf-token", default=None,
                   help="Optional Hugging Face token. Public CA streaming does not require one.")
    p.add_argument("--debug", action="store_true",
                   help="Short smoke-test run with fewer users/epochs.")
    p.add_argument("--debug-max-users", type=int, default=DEBUG_MAX_USERS_PER_SPLIT)
    p.add_argument("--debug-epochs", type=int, default=DEBUG_EPOCHS)
    p.add_argument("--rebuild-data-cache", action="store_true",
                   help="Delete compact CA cache before running.")
    p.add_argument("--rebuild-embedding-cache", action="store_true",
                   help="Delete frozen pair-embedding cache before running.")
    return p
...
def _resolve_device(value: str) -> str:
    ...
    if v.startswith("cuda") and not torch.cuda.is_available():
        raise RuntimeError(
            f"--device {value!r} requested, but torch.cuda.is_available() is False."
        )
    return value


def apply_cli_args(args: argparse.Namespace) -> None:
    """Apply terminal flags to the module-level experiment configuration.

    The research code below intentionally remains readable in terms of the original
    CEAR constants; this function is the single runtime override point.
    """
    global SEED, DEVICE, COUPLER, G_DIM, METRIC_RANK
    global EPOCHS, PATIENCE, MIN_DELTA, CHECKPOINT_EVERY
    global USERS_PER_BATCH, LR, WEIGHT_DECAY, EMBED_BATCH_SIZE
    global VAL_PERMUTATIONS, TEST_PERMUTATIONS, N_BOOTSTRAP, TEXT_ENCODER
    global CACHE_ROOT, RESULT_ROOT, HF_TOKEN
    global DEBUG, DEBUG_MAX_USERS_PER_SPLIT, DEBUG_EPOCHS

    SEED = int(args.seed)
    DEVICE = _resolve_device(args.device)
    COUPLER = str(args.coupler)
    G_DIM = int(args.g_dim)
    METRIC_RANK = int(args.metric_rank)
    EPOCHS = int(args.epochs)
    PATIENCE = int(args.patience)
    MIN_DELTA = float(args.min_delta)
    CHECKPOINT_EVERY = int(args.checkpoint_every)
    USERS_PER_BATCH = int(args.users_per_batch)
    LR = float(args.lr)
    WEIGHT_DECAY = float(args.weight_decay)
    EMBED_BATCH_SIZE = int(args.embed_batch_size)
    VAL_PERMUTATIONS = int(args.val_permutations)
    TEST_PERMUTATIONS = int(args.test_permutations)
    N_BOOTSTRAP = int(args.bootstrap)
    TEXT_ENCODER = str(args.text_encoder)
    CACHE_ROOT = _resolve_repo_path(args.cache_dir)
    RESULT_ROOT = _resolve_repo_path(args.result_dir)
    if args.hf_token:
        HF_TOKEN = args.hf_token

    DEBUG = bool(args.debug)
    DEBUG_MAX_USERS_PER_SPLIT = int(args.debug_max_users)
    DEBUG_EPOCHS = int(args.debug_epochs)

    if G_DIM < 1:
        raise ValueError("--g-dim must be >= 1")
    if METRIC_RANK < 1:
        raise ValueError("--metric-rank must be >= 1")
    if USERS_PER_BATCH < 1:
        raise ValueError("--users-per-batch must be >= 1")
    if PATIENCE < 0:
        raise ValueError("--patience must be >= 0")
    if CHECKPOINT_EVERY < 1:
        raise ValueError("--checkpoint-every must be >= 1")
    if VAL_PERMUTATIONS < 1 or TEST_PERMUTATIONS < 1:
        raise ValueError("Permutation counts must be >= 1")


def maybe_clear_requested_caches(args: argparse.Namespace) -> None:
    CACHE_ROOT.mkdir(parents=True, exist_ok=True)
    if args.rebuild_data_cache:
        for name in ("interactions.csv", "items.jsonl"):
            p = CACHE_ROOT / name
            if p.exists():
                print(f"Removing data cache: {p}")
                p.unlink()
    if args.rebuild_embedding_cache:
        for name in ("pair_embeddings.npy", "item_index.json", "embedding_meta.json"):
            p = CACHE_ROOT / name
            if p.exists():
                print(f"Removing embedding cache: {p}")
                p.unlink()
...
class CEARHumanStateModel(nn.Module):
    ...
    def predict(self, z_raw: torch.Tensor, g_prev: torch.Tensor):
        ...
        person_delta = scale * person
        logits = base + person_delta

        # Diagnostics only. Centered logits matter because a candidate-wise constant
        # does not affect the softmax. These let us distinguish harmless scale drift
        # from a genuinely growing person-specific correction.
        geo["base_logits"] = base
        geo["person_delta_logits"] = person_delta
        return logits, geo
...
def query_loss(
    ...
    logits, geo = model.predict(z, gq)

    with torch.no_grad():
        base = geo["base_logits"]
        person_delta = geo["person_delta_logits"]
        base_centered = base - base.mean(dim=-1, keepdim=True)
        person_centered = person_delta - person_delta.mean(dim=-1, keepdim=True)
        base_rms = torch.sqrt(base_centered.square().mean() + EPS)
        person_rms = torch.sqrt(person_centered.square().mean() + EPS)
        person_to_base = person_rms / (base_rms + EPS)

    stats = {
        "base_logit_rms": float(base_rms.item()),
        "person_correction_rms": float(person_rms.item()),
        "person_to_base_ratio": float(person_to_base.item()),
    }
    return F.cross_entropy(logits, y), logits, stats
...
def _save_epoch_checkpoint(
    ...
    torch.save({
        ...
        "person_scale": float(row["person_scale"]),
        "config": {
            "seed": SEED,
            "coupler": COUPLER,
            "g_dim": G_DIM,
...
def _save_latest_full_checkpoint(
    model: CEARHumanStateModel,
    ...
    """Overwritten each epoch; sufficient to resume the latest interrupted run."""
    payload = {
        "epoch": int(epoch),
        "state_dict": _cpu_state_dict(model),
    ...
    torch.save(payload, RESULT_ROOT / "latest_full.pt")
...
def train_model(
    ...
    if tensorboard_enabled:
        try:
            ...
            writer = SummaryWriter(log_dir=str(RESULT_ROOT / "tensorboard"))
    ...
    # Early stopping is optional. Replication default is PATIENCE=0, so all
    # requested epochs are completed while the true raw best checkpoint is kept.
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        base_rmses, person_rmses, person_ratios = [], [], []
        ...
        for start in range(0, len(order), USERS_PER_BATCH):
            ...
            base_rmses.append(q_stats["base_logit_rms"])
            person_rmses.append(q_stats["person_correction_rms"])
            person_ratios.append(q_stats["person_to_base_ratio"])
        ...
        row = {
            ...
            "train_base_logit_rms": float(np.mean(base_rmses)),
            "train_person_correction_rms": float(np.mean(person_rmses)),
            "train_person_to_base_ratio": float(np.mean(person_ratios)),
            "val_nll": val_nll,
        ...
        # Crash-safe epoch logging: CSV is refreshed every epoch, not only at the end.
        pd.DataFrame(hist).to_csv(history_path, index=False)

        # Save model-only weights every requested epoch. This is enough to reconstruct
        # g at, e.g., epoch 200 using the same deterministic split/support protocol.
        if epoch % CHECKPOINT_EVERY == 0 or epoch == n_epochs:
            _save_epoch_checkpoint(model, epoch, val_nll, row)

        # Keep one full resumable checkpoint without multiplying optimizer-state storage.
        # It is overwritten each epoch.
        if val_nll < best_val:
            best_val = val_nll
            best_epoch = epoch
        ...
        _save_latest_full_checkpoint(
            model, optimizer, rng, epoch, best_val, best_epoch
        )

        if writer is not None:
            writer.add_scalar("loss/train_query_ce", row["train_query_ce"], epoch)
            writer.add_scalar("loss/train_support_ce", row["train_support_ce"], epoch)
            writer.add_scalar("loss/val_nll", row["val_nll"], epoch)
            writer.add_scalar("state/g_norm", row["train_g_norm"], epoch)
            writer.add_scalar("person/scale", row["person_scale"], epoch)
            writer.add_scalar("person/correction_rms", row["train_person_correction_rms"], epoch)
            writer.add_scalar("person/correction_to_base", row["train_person_to_base_ratio"], epoch)
            writer.add_scalar("optimization/lr", row["lr"], epoch)
            writer.flush()

        print(
            f"epoch {epoch:03d} | trainQ={row['train_query_ce']:.4f} | "
            f"trainS={row['train_support_ce']:.4f} | val NLL={val_nll:.4f} | "
            f"person_scale={row['person_scale']:.3f} | "
            f"person/base={row['train_person_to_base_ratio']:.3f}"
        )

        # Optional early stopping. Raw best checkpoint above is still exact; this
        # separate criterion uses MIN_DELTA only to decide when to stop.
        if PATIENCE > 0:
            if val_nll < early_stop_best - MIN_DELTA:
                early_stop_best = val_nll
                bad_epochs = 0
            else:
                bad_epochs += 1
            if bad_epochs >= PATIENCE:
                print(
                    ...
                    f"raw best epoch={best_epoch}, val NLL={best_val:.6f}"
                )
                break
    ...
    # Final test always uses the raw best validation checkpoint.
    model.load_state_dict(best_state)
    return pd.DataFrame(hist), best_val, best_epoch
...
def main(args: Optional[argparse.Namespace] = None):
    ...
    apply_cli_args(args)
    seed_everything(SEED)
    maybe_clear_requested_caches(args)
    ...
    print("SEED:       ", SEED)
    print("EPOCHS:     ", DEBUG_EPOCHS if DEBUG else EPOCHS)
    print("BATCH users:", USERS_PER_BATCH)
    print("PATIENCE:   ", PATIENCE, "(0 = disabled)")
    print("CHECKPOINT: every", CHECKPOINT_EVERY, "epoch(s)")
    print("TensorBoard:", "off" if args.no_tensorboard else str(RESULT_ROOT / "tensorboard"))
    print("DEBUG:      ", DEBUG)
    ...
    # Train.
    history_df, best_val, best_epoch = train_model(
    ...
    # Console.
    print(f"\nBest validation checkpoint: epoch {best_epoch} | NLL={best_val:.6f}")
    print("\n" + "=" * 84)
